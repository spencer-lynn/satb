...
sdt_scenarios='baseline';
mood_conditions='neutral';

qc_threshold('DEV vgood sap2_army_5P.dat',mood_conditions,sdt_scenarios);
...
%         otherwise
%             try
%                 userinput=qc_threshold(userinput,mood_conditions,sdt_scenarios);
%                 
%             catch
%                 d.name
%                 userinput=input('Type a file name to process: ');
%                 
%             end
%     end %switch on userinput
% end %while running
...
function action=qc_threshold(subjectfilename,mood_conditions,sdt_scenarios)
global datapath outputPath datafile_suffix
global numStimuli numRatings stimNames
global lable_string delimiter fid columnheadings minimumRT chrDelimit %Variables available to all Data Processes
global condition scenario stimset block trialnum objnumber objname objtype behavior outcome payoff responsetime points
...
global cursor_info %response_gradient
cursor_info.DataIndex=[];
        ...
        for p=1:1
            ...
            if sum(isnan(response_gradient))<numStimuli %Do following only if current mood + param combo produced valid resonses for current subject (ie, was delivered and passes min RT tests)
                ...
                while ~done
                    switch action
                        case 'refresh'
                            response_gradient(cursor_info.DataIndex)=NaN
                            ...
                            [threshold gy gn]
                            linkdata on
                            %openvar('response_gradient') %doesn't work for fn workspaces, or as global (no update until fn closes.
                            action=input('Type an option [refresh|quit|fname]: ');
                        otherwise
                            done=true;
                    end %switch
                end %while not done
...
% [mycondition myscenario] %uncomment to troubleshoot
