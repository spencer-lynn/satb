function trigger_stimdur_test(stimdur,loopflag)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%
%VERSION
% trigger_stimdur_test, 7/31/12
% Spencer K. Lynn, spencer.lynn@gmail.com
%
%CHANGE LOG
% 7/31/12 
% - forked from satb_stimdur_test, to add digital event triggers
% 
%
%USES
...
% See trigger_demo.m for more.
% triggerinfo.Event_Code becomes conditions.trigger_ID in get_signal_triggers.m
...
triggerinfo=read_list(strcat(listfolder,'trigger_list.txt')); %Text file describes trigger names, codes, ports, durations. Used by fetch_signal, accrue_payoffs, etc.
for i=1:length(triggerinfo)
    % Dereferece the port ID strings in trigger to replace with the port uddobj/dioobj value.
    triggerinfo(i).Trigger_Port=eval(triggerinfo(i).Trigger_Port{1});
end
disp('Variable: triggerinfo(1)')
triggerinfo(1)
...
trialinfo=read_list(strcat(listfolder,'trials_trigger_list.txt')); %Read information about each trial (getsignal xfn, response-code place-holder, duration)
...
[response startRT endRT]=show_stim_trigger(stiminfo(mytrial)); %show main stim
...
% OFF         [response rwOpenTime endRT]=show_stim(responseWindowScreen(1)); %end stim pres with response window
        ...
        trigger_onset=trigger_port(triggerinfo(13)); %For testing: Mark end of stim-duration w/ trigger #13 in trigger_list
...
% OFF        actual_stimdur=rwOpenTime-startRT;
        ...
        stimonset_trigger_lag=NaN; %not coded, see zm expt.
        outcome_trigger_lag=NaN; %not coded, see zm expt.
        payoff_trigger_lag=NaN; %not coded, see zm expt.
