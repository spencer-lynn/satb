function tuned_stim_dur=tune_stimdur(target_info,following_info)
%SAtb FUNCTION TUNE_STIMDUR(target_info,following_info)
% 
...
conditions.xtrial=1;show_stim(target_info(1)); %get m-files into memory.
...
while stimulus_duration(counter)>rounded_target_stim_dur
    ...
    %making sequential calls to show_stim.m (show_stim doesn't flip away at
    %end of stimdur).
    %     
    ...
    %     So, it's the loading->flip of the following stim (eg, mask) that
    %     causes an inadvertant increase in target's duration on-screen.
    
%     ml_time=tic; %VERBOSE uncomment to see ML's rough estimate (doesn't include processes prior to next flip).
    [~, targetStartTime, ~]=show_stim(target_info(stimnum)); %show main stim
%     toc(ml_time) %VERBOSE uncomment to see ML's rough estimate (doesn't include processes prior to next flip).

%If no following_info provided, follow target with a flip-to-blank. tic/toc shows the if statement only add fraction of ms to overall time.
if isfield(following_info,'Stimulus')
    [~, followStartTime, ~]=show_stim(following_info(1)); %show following stim
else
